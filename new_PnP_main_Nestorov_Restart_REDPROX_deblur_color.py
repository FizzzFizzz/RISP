...
class PnP(nn.Module):
    ...
    def forward(self, kernel, initial_uv, f, clean, sigma=25.5, lamb=690, sigma2=1.0, denoisor_sigma=25): 
        ...
        for k in range(self.nb):
            ...
            k_min=5
            crit = torch.mean(torch.abs(x-oldx)).cpu().numpy()

            if (k>k_min) and (crit<oldcrit):
                j = 1
                print('Restart at iteration: ', k)
            else:
                j = j+1
        ...
        return torch.clamp(w, min = -0., max =255.) 
...
def plot_psnr(denoisor_level, lamb, sigma):
    ...
    savepth = 'images_Nesterov_Restart/'
    ...
    plt.ylabel('PSNR')
    # plt.show()
    plt.savefig('PSNR_level{}_lamb{}_Nestorov_Restart.png'.format(denoisor_level, lamb))
    plt.close()
...
# REDI-Prox 0004.png, kernel 6. step = 0.2, output = w_, fine tuned
plot_psnr(0.1, 3.6, 12.75) # 27.28, chosen
